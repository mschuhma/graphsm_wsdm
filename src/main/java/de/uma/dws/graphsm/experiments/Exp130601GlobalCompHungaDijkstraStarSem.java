...
public class Exp130601GlobalCompHungaDijkstraStarSem {
	...
	public final static Logger log = LoggerFactory
			.getLogger(Exp130601GlobalCompHungaDijkstraStarSem.class);
	...
	public static void runStarSem2012(String graphDir) {
		System.out
				.println("MainRunParallel in de.uma.dws.graphsm.neo4j.compare now running...");
		System.out.println("Check log files in /log");

		// String graphDir = "/home/mschuhma/graph-starsem-2hops.db";

		Neo4jRdfGraph graph = Neo4jRdfGraph.getInstance(graphDir, false);

		ExecutorService pool = Executors.newFixedThreadPool(conf.getInt(
				"system.parallel.threads", 1));
		...
		Double maxPathLen = 99999.0;

		try {
			for (int i = 1; i <= 750 * 2; i += 2) {

				int j = i + 1;
				log.debug(
						"Compute similarity between snippet {} and {} with dijkstra on weighted graph",
						i, j);

				simScoreResults.add(pool.submit(new DocCompDijkstraWrapper(
						graph, new Snippet(0, i, "http://dummy", "dummy",
								"dummy"), new Snippet(0, j, "http://dummy",
								"dummy", "dummy"), maxPathLen,
						globalMaxPathCost)));
			}
			...
			log.info("globalMaxPathCost = {}",
					Double.longBitsToDouble(globalMaxPathCost.get()));
			System.out
					.println("MainRunParallel threads finished. Shutting down database and thread pool.");
